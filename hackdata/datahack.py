...
all = ['An user attempted to change his password', 'A directory service object was deleted', 'A directory service object was created',
       'A directory service object was modified', 'User account disabled or deleted', 'User account unlocked', 'User account enabled or created']
Task_eventname = ['User account changed', 'A directory service object was moved', 'A directory service object was deleted', 'A directory service object was created',
                  'A directory service object was modified', 'User account disabled or deleted', 'User account unlocked', 'User account enabled or created']
Task_39_eventname = [' A user account was changed by a computer account', 'User account changed', 'A directory service object was moved', 'A directory service object was deleted', 'A directory service object was created', 'A directory service object was modified', 'User account disabled or deleted', 'User account unlocked',
                     'User account enabled or created', 'A user account was changed by another user account', 'An user failed to change his password']
Daily_rpt_event = ['User account disabled or deleted', 'Multiple Windows Logon Failures', 'Security Enabled Local Group Changed',
                   'Multiple Windows Logon Failures', 'Logon Failure - Unknown user or bad password', 'User account enabled or created']
...
Daily_rpt_view = ['Multiple Windows Logon Failure',
                  'Security Enabled Local Group Changed', 'User account created/deleted']
...
def excelmerge(s_path):
    print(s_path)
    cwd =os.getcwd()
    my_dict = {}
    excel_p=f"{s_path}\excel files"
    ...
    for filename in os.listdir(s_path):
        ...
        print(filename)
        if ext == ".csv":
            df = pd.read_csv(os.path.join(s_path, filename),  sep=';' , encoding="utf-8")
            new_filename=f"{filename}.xlsx"
            df=df.to_excel(new_filename,index=False)
            print(new_filename)
            try:
    ...
    for val in os.listdir(excel_p):
        ...
        key = val[:30]   # Extract the first four characters as the key
        if key == val[:30] and ext== '.xlsx':
            if key == val[:30] and ext== '.xlsx':   # Check if the key is similar to the value
